...
export function getSecret(name, runtimeEnv) {
...
function formatExpression(inputString) {
    const isJSON = /^\s*(?:\(\s*)?(?:\{[\s\S]*\}|\[[\s\S]*\]|`[^`]*`|'[^']*'|"[^"]*"|true|false|null|-?\d+(\.\d+)?([eE][+\-]?\d+)?)?(?:\s*\))?\s*$/.test(inputString.trim());
    if (isJSON || !isMultiline(inputString))
        return `async (ctx,getSecret,projectEnv,stringify) => (${inputString})`;
    return `async (ctx,getSecret,projectEnv,stringify) => {${inputString}}`;
}
export async function parseExpression(ctx, val) {
    ctx = { root: ctx };
    const runtimeEnv = process.env.runtimeEnv ?? "test-env";
    return eval(formatExpression(val))({ ...ctx, env: {} }, (name) => getSecret(name, runtimeEnv), JSON.parse((runtimeEnv ? process.env[`${runtimeEnv}ProjectEnv`] : process.env.projectEnv) ?? "{}"), (value) => (typeof value === "string" ? value : JSON.stringify(value)));
}
export const scripExecutor = (workflowDirectory) => async (nodeId, args, root, subNodes = {}) => {
    const { default: { default: script }, } = await import(`../../scripts/${nodeId}.cjs`);
    const { nodes } = await import(path.join(workflowDirectory, "nodes.js"));
    const node = nodes.find((node) => node.id === nodeId);
    if (!node) {
        throw new Error(`Node with ID ${nodeId} not found.`);
    }
    const subNodesCamelcased = node.nodes?.map((subNode) => ({
        ...subNode,
    ...
    return script(args, {
        ...
        auth: {
            getKey: (options) => {
                ...
                if (groupUid !== node._groupInfo.uid) {
                    throw new Error(`Selected key (${selectedKey}) does not belong to this trigger's group (${node._groupInfo.uid}).`);
                }
                return JSON.parse(process.env[runtimeEnv ? `${runtimeEnv}ProjectEnv` : "projectEnv"] || "{}")[selectedKey];
            },
            getToken: () => {
        ...
        execute: async (name, args) => {
            const nodeToExecuteId = subNodesCamelcased.find((subNode) => subNode.label === name)?.id;
            if (!nodeToExecuteId) {
...
export function setResult(obj, value, keyPath) {
...
export async function executeWorkflow(workflowName, args) {
