-- Migration: fix_get_edition_details_condition_column
-- Updates get_edition_details to remove reference to si.condition and use only cs.standard_name for condition_name

DROP FUNCTION IF EXISTS public.get_edition_details(uuid, uuid);

CREATE OR REPLACE FUNCTION public.get_edition_details(
  p_edition_id uuid,
  ...
  SELECT
    jsonb_build_object(
      ...
      'stock_items', (
        ...
        WHERE si.edition_id = e.edition_id AND si.organization_id = p_organization_id
      )
    )
  INTO result
  FROM editions e
