-- Migration: implement_keyset_pagination
-- Drops and recreates the search_inventory function to use keyset pagination for stable pagination results
...
WHERE si.organization_id = p_org_id
  AND (p_search_query = '' OR b.title ILIKE '%' || p_search_query || '%' OR e.isbn13_ol ILIKE '%' || p_search_query || '%')
...
WHERE
  ...
  (p_last_date_added IS NULL OR es.max_date_added < p_last_date_added)
  OR (es.max_date_added = p_last_date_added AND es.edition_id > p_last_edition_id)
ORDER BY
  es.max_date_added DESC, -- Primary sort key
  es.edition_id ASC      -- Tie-breaker for stable ordering
LIMIT p_limit_count;
END;
$$; 